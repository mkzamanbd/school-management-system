...
{
$aColumns = array( 's.id','s.sname','s.admissionNo','s.balance','s.fees','b.branch','s.fname','s.aprilb','s.julyb','s.octoberb','s.januaryb');
	...
	if(isset($_GET['admissionNo']) && $_GET['admissionNo']!="")
	{
	$condArr[] = "s.admissionNo like '%".mysqli_real_escape_string($conn,$_GET['admissionNo'])."%'";
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
                    html_entity_decode($aRow['sname'].'<br/>'.$aRow['fname']),
                    $aRow['branch'],
					$aRow['admissionNo'],
                    $aRow['fees'], 
					$aRow['balance'],
					...
					html_entity_decode('<button class="btn btn-warning btn-xs" onclick="javascript:GetFeeForm('.$aRow['id'].')"><i class="fa fa-inr "></i>  Take  </button>')
...
$aColumns = array( 's.id','s.sname','s.admissionNo','s.balance','s.fees','b.branch','s.fname','s.aprilb','s.julyb','s.octoberb','s.januaryb');
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
					...
					html_entity_decode('<button class="btn btn-warning btn-xs" onclick="javascript:GetFeeForm('.$aRow['id'].')"><i class="fa fa-usd "></i> TC Generate </button>')
